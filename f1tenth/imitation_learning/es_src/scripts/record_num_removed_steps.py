...
cbf_triggered_steps_to_remove = 90

# Saving settings
path = 'models/cbf_triggered_steps_to_remove/90'
...
for iter in range(n_iter):
    ...
    while not done and laptime < 8:
        ...
        while not done and tracker_count < lane_switcher_conf.tracker_steps:
            ...
            ego_scan = agent_utils.downsample_and_extract_lidar(obs, 0, 108, downsampling_method)
            ego_x = obs['poses_x'][0]
            ego_y = obs['poses_y'][0]
    ...
    if (iter % save_skip_interval == 0) and (iter > 0):
        ...
        print("length of iteration:", len(data_removal_log["Iteration"]))
        print("length of number of steps removed:", len(data_removal_log["Number of Steps Removed"]))
        print("length of total steps before removal:", len(data_removal_log["Total Steps Before Removal"]))
        print("length of total steps after removal:", len(data_removal_log["Total Steps After Removal"]))
        print("length of ratio of steps removed:", len(data_removal_log["Ratio of Steps Removed"]))

        
        df = pd.DataFrame(data_removal_log)
        log_path = Path(path+f'/step_removal_data_log/steps_removal_{int(iter)}.csv')
