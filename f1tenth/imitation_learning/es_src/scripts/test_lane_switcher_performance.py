...
for i in range(num_iter):
    ...
    while not done and laptime < 8:
        ...
        while not done and tracker_count < lane_switcher_conf.tracker_steps:
            ...
            if start_perturb:
                action = np.array([[-ego_steer, ego_speed], [opp_steer, opp_speed*0.7]])
                num_pertub_steps += 1
            else:
                action = np.array([[ego_steer, ego_speed], [opp_steer, opp_speed*0.7]])
            ...
            tracker_count += 1
            if render_sim:
                env.render('debug', debug_scale_factor=0.01)
                vl1.delete()
                vl2.delete()
                vl3.delete()
    ...
    if num_pertub_steps > 0:
        num_pertub_steps_list.append(num_pertub_steps)
        print("Number of perturb steps in current iteration: ", num_pertub_steps)
                
    # print("finish %d episode", i)
...
print("Percentage of iterations that overtake: ", overtake_rate)

print("------------------------")
print("Average number of perturb steps: ", np.mean(num_pertub_steps_list))
print("Std of number of perturb steps: ", np.std(num_pertub_steps_list))
print("Median of number of perturb steps: ", np.median(num_pertub_steps_list))
print("Min of number of perturb steps: ", np.min(num_pertub_steps_list))
print("Max of number of perturb steps: ", np.max(num_pertub_steps_list))
import statistics as stats
print("Mode of number of perturb steps: ", stats.mode(num_pertub_steps_list))