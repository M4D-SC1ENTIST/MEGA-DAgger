...
def eval_model_performance(model_path):
...
def eval_all_models_in_folder(source_path):
    log = {'model_idx': [], 'avg_collision_rate': [], 'avg_overtake_rate': []}
    for idx in range(10):
        model_idx = str(idx+1)+str(0)+str(0)
        model_path_dir = source_path + '/iter_' + str(model_idx) + '_model.pkl'
        model_path = Path(model_path_dir)
        if model_path.is_file():
            collision_rate, overtake_rate = eval_model_performance(model_path)
            log['model_idx'].append(model_idx)
            log['avg_collision_rate'].append(collision_rate)
            log['avg_overtake_rate'].append(overtake_rate)
            print("Model idx: ", model_idx, " collision rate: ", collision_rate, " overtake rate: ", overtake_rate)
        else:
            print("Model idx: ", model_idx, " does not exist")
    return log


if __name__ == '__main__':
    source_path = 'models/training_performance_comp/new/map0/at5/multi_with_filter'
    #source_path = 'models/fixed_training_dataset_length_comparison/1.0/random_truncate'
    
    log = eval_all_models_in_folder(source_path)
    test_data_dir = 'test_data/training_performance_comp/new/map0/at5/'
    
    test_data_file_dir = test_data_dir + 'multi_with_filter.csv'
    #test_data_file_dir = test_data_dir + '1.0_random_truncate.csv'
    
    test_data_path = Path(test_data_file_dir)
    df = pd.DataFrame(log)
