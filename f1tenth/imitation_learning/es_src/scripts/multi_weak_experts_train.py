...
# Saving settings
use_CBF_filter = True
save_folder = 'training_performance_comp/map0/at1/multi_with_filter'
...
def conflict_check_scoring_and_resolution(curr_scan, expert_action, curr_vel_x, curr_vel_y, CBF_value, global_dataset, similarity_threshold=0.9995, alpha=0.5):
    ...
    #       4. Calculate the min max norm of speed and CBF_value
    speed_concat = np.array(data_entries_above_thresh['speed'])
    speed_concat = np.append(speed_concat, curr_speed)
    speed_norm = np.max(speed_concat) - np.min(speed_concat)
    CBF_value_concat = np.array(data_entries_above_thresh['CBF_value'])
    CBF_value_concat = np.append(CBF_value_concat, CBF_value)
    CBF_value_norm = np.max(CBF_value_concat) - np.min(CBF_value_concat)

    #       5. Add normalized speed and CBF_value together with a weighting factor.
    for idx2 in range(len(data_entries_above_thresh['idx'])):
        curr_entry_normalized_speed = data_entries_above_thresh['speed'][idx2] - np.min(speed_concat) / speed_norm
        curr_entry_normalized_CBF_value = data_entries_above_thresh['CBF_value'][idx2] - np.min(CBF_value_concat) / CBF_value_norm
                    ...
                    if use_CBF_filter:
                        ...
                        if online_ego_oppo_collision_check:
                            ...
                            if step_num >= 1:
                                ...
                                dataset = conflict_check_scoring_and_resolution(scan, expert_ego_action, curr_vel_x, curr_vel_y, CBF_value, dataset)
                        ...
                        if step_num_left >= 1:
                            ...
                            curr_vel_x = obs["linear_vels_x"][0]
                            curr_vel_y = obs["linear_vels_y"][0]
                            dataset = conflict_check_scoring_and_resolution(scan, expert_ego_action, curr_vel_x, curr_vel_y, CBF_value_border, dataset)
