...
def get_collision_points(model_path):
    log = {'collided_pose_x': [], 'collided_pose_y': []}
            ...
            while not done and tracker_count < lane_switcher_conf.tracker_steps:
                ...
                if np.any(obs['collisions']):
                    if (obs['collisions'][0] == 1.0) and (obs['collisions'][1] == 0.0):
                        ...
                        log['collided_pose_x'].append(obs['poses_x'][0])
                        log['collided_pose_y'].append(obs['poses_y'][0])
...
if __name__ == '__main__':
    model_path_dir = 'models/single_weak_expert_seed7682/0.5/70_removed_steps_cbf_filter/iter_400_model.pkl'
    test_data_dir = 'test_data/collision_pts/single_weak_expert_seed7682_collision_pts.csv'

    model_path = Path(model_path_dir)
    log = get_collision_points(model_path)
