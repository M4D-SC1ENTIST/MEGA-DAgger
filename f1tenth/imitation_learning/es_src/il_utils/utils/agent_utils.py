...
from il_utils.utils import downsampling


def downsample_and_extract_lidar(observ, id, observation_shape, downsampling_method):
    """
    Down-sample the lidar data and extracts the relevant features.
    """
    # print("observ: ", observ)
    lidar_scan = observ["scans"][id]
    if len(lidar_scan) != 108:
        processed_lidar_scan = downsampling.downsample(lidar_scan, observation_shape, downsampling_method)
    else:
        processed_lidar_scan = lidar_scan
    return processed_lidar_scan


def sample_traj(env, policy, start_pose, max_traj_len, observation_shape=108, downsampling_method="simple", render=True,
                render_mode="human_fast", for_eval=False):
    """
...
def sample_trajs(env, policy, start_pose, max_traj_len, n_trajs, observation_shape, downsampling_method, render,
                 render_mode, for_eval=False):
    """
    Samples `n_trajs` trajectories by repeatedly calling sample_traj().
    """
    if for_eval:
        data = {"observs": [], "poses_x": [], "poses_y": [], "poses_theta": [], "scans": [], "actions": [],
                "rewards": [], "travelled_distances": []}
    else:
        data = {"observs": [], "poses_x": [], "poses_y": [], "poses_theta": [], "scans": [], "actions": [],
                "rewards": []}

    for _ in range(n_trajs):
        traj = sample_traj(env, policy, start_pose, max_traj_len, observation_shape, downsampling_method, render,
                           render_mode, for_eval)
...
def eval(env, policy, start_pose, max_traj_len, eval_batch_size, observation_shape, downsampling_method, render,
         render_mode):
    """
    Evaluates the performance of a policy over `eval_batch_size` trajectories.
    """
    eval_res = sample_trajs(env, policy, start_pose, max_traj_len, eval_batch_size, observation_shape,
                            downsampling_method, render, render_mode, for_eval=True)
